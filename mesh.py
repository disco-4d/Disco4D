...
class Mesh:
    ...
    def load(cls, path=None, resize=True, renormal=True, retex=False, front_dir='+z', **kwargs):
        ...
        if retex or (mesh.albedo is not None and mesh.vt is None):
            ...
            mesh.auto_uv(cache_path=path, vmap=False)
    ...
    def write_obj(self, path):
        # Sanitize file paths to avoid issues with special characters
        base_name = self.sanitize_filename(os.path.basename(path).replace(".obj", ""))
        
        mtl_path = os.path.join(os.path.dirname(path), f"{base_name}.mtl")
        albedo_path = os.path.join(os.path.dirname(path), f"{base_name}_albedo.png")
