...
def retrieve_local_coord_system(vertices, faces, fidxs):
    # triangle_verts = vertices[faces].float()
    ...
    face_vertices = vertices[faces].float()[fidxs]
    v1 = face_vertices[:, 0]
    v2 = face_vertices[:, 1]
    v3 = face_vertices[:, 2]
    ...
    edges1 = v2 - v1
    edges2 = v3 - v1
    normal = torch.cross(edges1, edges2, dim=1)
    normal = normal / torch.norm(normal, dim=1, keepdim=True)

    # Define the X-axis (first edge normalized)
    x_axis = edges1 / torch.norm(edges1, dim=1, keepdim=True)

    # Compute the Y-axis using cross product of normal and x_axis
