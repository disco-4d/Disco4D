...
# -----------------------------------------------------------------------------
# Geometry helpers (formerly imported from lib.gs_utils.gs_renderer_pose_smplx)
# -----------------------------------------------------------------------------
# These five small utilities are all that mesh_to_gs needs from the 3.4k-line
# pose-smplx renderer; inlining them lets us drop that dependency.

# One gaussian per triangle, placed at the centroid (barycentric 1/3,1/3,1/3).
_BARY_COORDS = torch.tensor([[1/3, 1/3, 1/3]], dtype=torch.float32)


def find_points_inside_triangle(triangles):
    """Sample one point at the centroid of each triangle. triangles: (F, 3, 3)."""
    device = triangles.device if triangles.is_cuda else 'cuda'
    faces_verts = triangles.to(device)
    bary = _BARY_COORDS.to(device)[..., None]
    points = (faces_verts[:, None] * bary[None]).sum(dim=-2)
    return points.reshape(-1, 3)


def calculate_sides_and_heights(triangles):
    """Returns (first-side length, height to first side) per triangle via Heron's."""
    side_lengths = torch.norm(triangles - torch.roll(triangles, -1, dims=1), dim=2)
...
def batch_vector_to_quaternion(normals):
    """Quaternion (xyz, w) that rotates +z to each (normalized) normal."""
    ...
    sin_h = torch.sin(angle / 2).unsqueeze(-1)
    cos_h = torch.cos(angle / 2).unsqueeze(-1)

    quaternion = torch.zeros(normals.size(0), 4, dtype=torch.float32, device=device)
    quaternion[:, :3] = rotation_axis / rotation_axis_norm * sin_h
    quaternion[:, 3] = cos_h.squeeze()
