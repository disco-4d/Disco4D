...
class GaussianModel:
    ...
    def get_opacity(self):
        ...
        clothing_fidxs = self.sample_fidxs
        smplx_fidxs = self.sample_fidxs_smplx
        occ_fidxs = torch.unique(clothing_fidxs) 
        occ_list = occ_fidxs.tolist()
        fill_list = torch.ones_like(smplx_fidxs).float()
        fill_list[occ_list] = inverse_sigmoid(torch.zeros_like(fill_list[occ_list]))
        opacity_smplx = self._opacity_smplx * fill_list.unsqueeze(-1)
    ...
    def update_canonical_smplx_colors(self, idx):
        # Read PLY file only once
        features_dc = self.features_dc_list[idx]
        features_extra = self.features_dc_list[idx]

        # **Instead of creating new tensors, update existing ones**
        with torch.no_grad():
            self._features_dc_smplx.copy_(torch.tensor(features_dc, dtype=torch.float, device=self.device).transpose(1, 2))
            self._features_rest_smplx.copy_(torch.tensor(features_extra, dtype=torch.float, device=self.device).transpose(1, 2))
    
    def load_canonical_smplx_colors(self, input_ply_list):
        """ Preloads PLY color data for fast updates. """
        self.features_dc_list = []
        self.features_rest_list = []

        for input_ply in input_ply_list:
            plydata = PlyData.read(input_ply)
            num_points = self._features_dc_smplx.shape[0]
            ...
            features_extra = features_extra.reshape((num_points, 3, (self.max_sh_degree + 1) ** 2 - 1))

            # Store in lists for fast access
            self.features_dc_list.append(features_dc)
            self.features_rest_list.append(features_extra)
    ...
    def create_from_canonical_smplx_winput(self, cano_mesh, input_ply, sample_fidxs=None, sample_bary=None):
        ...
        self.n_smplx_points = num_points 
    ...
    def save_ply_deformation_fidxs(self, path):
        ...
        means3D = self.get_learnable_xyz
        ...
        if USE_HACK:
            ...
            time = time.float()
        opacity = self._opacity
        scales = self.get_learnable_scaling_raw
        rotations = self.get_learnable_rotation_raw

        means3D_deform, scales_deform, rotations_deform, opacity_deform = self._deformation(means3D, scales, 
                                                            rotations, opacity,
                                                            time) #  time is not none

        xyz =  means3D_deform.detach().cpu().numpy()
        normals = np.zeros_like(xyz)
        rotations_final =  rotations_deform
        scales_final =  scales_deform # .detach().cpu().numpy()
        opacity_final = opacity_deform

        scale = self.scaling_inverse_activation(self.scaling_activation(scales_final)).detach().cpu().numpy()
        rotation = self.rotation_activation(rotations_final).detach().cpu().numpy()
        opacities = self.inverse_opacity_activation(self.opacity_activation(opacity_final)).detach().cpu().numpy()
