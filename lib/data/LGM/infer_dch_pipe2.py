...
# sys.path.insert(0, osp.join('..', 'LGM', 'core'))
...
ckpt_path = "lib/data/LGM/pretrained/model_fp16_fixrot.safetensors"
opt = Options(
    input_size=256,
...
# process function
def process(opt: Options, paths, name):
    ...
    for img_path in paths:
        ...
        zoom_factor = 256/ input_image.shape[1]
        image_resized = zoom(input_image, (zoom_factor, zoom_factor, 1))

        mv_inputs.append(image_resized)
    
    mv_image = np.stack(mv_inputs, axis=0) # [4, 256, 256, 3], float32
    ...
    for i in range(4):
        im = mv_image[i]
        # outpath = 'workspace/vis_data'
        os.makedirs(f'{opt.workspace}/{name}', exist_ok=True)
        Image.fromarray((im* 255).astype(np.uint8)).save(f'{opt.workspace}/{name}/{i}.png')
