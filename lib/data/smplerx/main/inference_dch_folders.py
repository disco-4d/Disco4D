...
import torch
sys.path.insert(0, osp.join('..', 'smplerx'))
sys.path.insert(0, osp.join('..', 'smplerx', 'main'))
sys.path.insert(0, osp.join('..', 'smplerx', 'data'))
sys.path.insert(0, osp.join('..', 'smplerx', 'common'))
from smplerx.main.config import cfg
import cv2
from tqdm import tqdm
import json
from typing import Literal, Union
from mmdet.apis import init_detector, inference_detector
from smplerx.common.utils.inference_utils import process_mmdet_results, non_max_suppression
...
def infer_dch(img_path_list, output_folder_list):



    # args = parse_args()
    args_pretrained_model = 'smpler_x_h32_correct'
    args_testset = 'EHF'
    args_agora_benchmark = 'agora_model'
    args_num_gpus = 1
    args_exp_name = 'output/inference/inference_video'
    multi_person = True
    args_save_mesh = True
    args_show_bbox = True 
    args_show_verts = True
    args_iou_thr = 0.5
    args_bbox_thr = 50


    config_path = osp.join('./smplerx/main/config', f'config_{args_pretrained_model}.py')
    ckpt_path = osp.join('./smplerx/pretrained_models', f'{args_pretrained_model}.pth.tar')

    cfg.get_config_fromfile(config_path)
    cfg.update_test_config(args_testset, args_agora_benchmark, shapy_eval_split=None, 
                            pretrained_model_path=ckpt_path, use_cache=False)
    cfg.update_config(args_num_gpus, args_exp_name)
    ...
    checkpoint_file = './smplerx/pretrained_models/mmdet/faster_rcnn_r50_fpn_1x_coco_20200130-047c8118.pth'
    # config_file= './mmpose/pretrained_models/mmdet/mmdet_faster_rcnn_r50_fpn_coco.py'
    config_file= './mmpose/demo/mmdetection_cfg/faster_rcnn_r50_fpn_coco.py'
    model = init_detector(config_file, checkpoint_file, device='cuda:0')  # or device='cuda:0'
    # model = DetInferencer(model='faster-rcnn_r50_fpn_1x_coco', weights=checkpoint_file)

    for args_img_path, args_output_folder in zip(img_path_list, output_folder_list):
    # args_img_path = img_path
    # args_output_folder = output_folder

        frame_names = sorted(os.listdir(args_img_path))
        num_frames= len(frame_names)
        # for frame in tqdm(range(start, end)):
        for frame in tqdm(range(num_frames)):
            # img_path = os.path.join(args.img_path, f'{int(frame):06d}.jpg')
            # img_path = args.img_path
            img_path = os.path.join(args_img_path, frame_names[frame])
            ...
            if len(mmdet_box[0])<1:
                # save rendered image
                frame_name = img_path.split('/')[-1]
                save_path_img = os.path.join(args_output_folder, 'img')
            ...
            #     mmdet_box = non_max_suppression(mmdet_box[0], args_iou_thr)
            #     num_bbox = len(mmdet_box)
            ...
            for bbox_id in range(num_bbox):
                ...
                if mmdet_box_xywh[2] < args_bbox_thr or mmdet_box_xywh[3] < args_bbox_thr * 3:
                    continue
                ...
                ## save mesh
                if args_save_mesh:
                    save_path_mesh = os.path.join(args_output_folder, 'mesh')
                ...
                save_path_smplx = os.path.join(args_output_folder, 'smplx')
                os.makedirs(save_path_smplx, exist_ok= True)

                npz_path = os.path.join(save_path_smplx, f'{frame_name}_{bbox_id}.npz')
                ...
                vis_img = render_mesh(vis_img, mesh, smpl_x.face, {'focal': focal, 'princpt': princpt}, 
                                    mesh_as_vertices=args_show_verts)
                if args_show_bbox:
                ...
                save_path_meta = os.path.join(args_output_folder, 'meta')
                os.makedirs(save_path_meta, exist_ok= True)
                with open(os.path.join(save_path_meta, f'{frame_name}_{bbox_id}.json'), "w") as outfile:
