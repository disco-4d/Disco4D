...
from lib.gs_4d_utils.gaussian_model_4d_canonical import GaussianModel, BasicPointCloud, load_from_smplx_path, load_from_smplx_obj
...
class Renderer:
    def __init__(self, sh_degree=3, white_background=True, radius=1):
        ...
        from lib.arguments import ModelHiddenParams
    ...
    def initialize_from_smplx_gaussians(self, input=None, input_mesh=None, lgm_ply=None, reinitialize=False):
        if input_mesh.endswith(('.obj')):
            verts, faces, face_normals = load_from_smplx_obj(input_mesh)
        elif input_mesh.endswith(('.npz')):
            verts, faces, face_normals = load_from_smplx_path(input_mesh)
        ...
        nonskin_categories = [2, 3, 4, 5, 6, 7, 8, 9, 10]
    ...
    def render(
        ...
        if compute_cov3D_python:
            ...
            scales = self.gaussians.get_learnable_scaling_raw
            rotations = self.gaussians.get_learnable_rotation_raw
        ...
        # combine with smplx
        means3D_final =torch.cat((self.gaussians.get_smplx_xyz, means3D_final)) #  means3D_final
        scales_final = torch.cat((self.gaussians.get_smplx_scaling, scales_final))
        rotations_final = torch.cat((self.gaussians.get_smplx_rotation, rotations_final))
        opacity = torch.cat((self.gaussians.get_smplx_opacity, opacity))
    ...
    def render_learnable_w_deformation(
        self,
        viewpoint_camera,
        scaling_modifier=1.0,
        bg_color=None,
